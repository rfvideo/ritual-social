...
/**
 * Fixed gas limits, generous overestimates for each write function.
 *
 * Ritual Chain's RPC appears not to fully support `eth_estimateGas` (it
 * returns a generic "Internal JSON-RPC error" when wallets try to estimate
 * gas automatically before sending). Supplying an explicit `gas` value skips
 * that estimation call entirely — the wallet just uses the number we give it.
 * Unused gas is refunded as normal, so overestimating here is safe.
 */
